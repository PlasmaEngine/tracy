...
#  define TracyCZone( ctx, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { NULL, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), TRACY_CALLSTACK, active );
#  define TracyCZoneN( ctx, name, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), TRACY_CALLSTACK, active );
#  define TracyCZoneC( ctx, color, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { NULL, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), TRACY_CALLSTACK, active );
#  define TracyCZoneNC( ctx, name, color, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), TRACY_CALLSTACK, active );
#else
#  define TracyCZone( ctx, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { NULL, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), active );
#  define TracyCZoneN( ctx, name, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), active );
#  define TracyCZoneC( ctx, color, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { NULL, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), active );
#  define TracyCZoneNC( ctx, name, color, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), active );
#endif
...
#  define TracyCZoneS( ctx, depth, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { NULL, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), depth, active );
#  define TracyCZoneNS( ctx, name, depth, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), depth, active );
#  define TracyCZoneCS( ctx, color, depth, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { NULL, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), depth, active );
#  define TracyCZoneNCS( ctx, name, color, depth, active ) static const struct ___tracy_source_location_data TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE) = { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack( &TracyConcat(__tracy_source_location,TRACY_SOURCE_LOCATION_LINE), depth, active );
