...
#  define TracyGpuNamedZone( varname, name, active ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE) { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; tracy::GpuCtxScope varname( &TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE), TRACY_CALLSTACK, active );
#  define TracyGpuNamedZoneC( varname, name, color, active ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE) { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; tracy::GpuCtxScope varname( &TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE), TRACY_CALLSTACK, active );
...
#else
#  define TracyGpuNamedZone( varname, name, active ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE) { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; tracy::GpuCtxScope varname( &TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE), active );
#  define TracyGpuNamedZoneC( varname, name, color, active ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE) { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; tracy::GpuCtxScope varname( &TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE), active );
...
#  define TracyGpuNamedZoneS( varname, name, depth, active ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE) { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, 0 }; tracy::GpuCtxScope varname( &TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE), depth, active );
#  define TracyGpuNamedZoneCS( varname, name, color, depth, active ) static const tracy::SourceLocationData TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE) { name, TRACY_SOURCE_LOCATION_FUNCTION,  TRACY_SOURCE_LOCATION_FILE, (uint32_t)TRACY_SOURCE_LOCATION_LINE, color }; tracy::GpuCtxScope varname( &TracyConcat(__tracy_gpu_source_location,TRACY_SOURCE_LOCATION_LINE), depth, active );
